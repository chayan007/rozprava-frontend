...
export const userService = { settings, getProfile, getRecommendations, followUser, getFollowers};
...
function getFollowers(flag){
    ...
    const url =  stringFormat(`${config.commonConfig.$apiUrl}/${config.userConfig.api.getFollowers.endpoint}`, followerFlag );
    ...
    return axios.get(url, { headers: authenticationHeader })
        ...
        .catch(() => {
            throw stringFormat(config.messagingConfig.messages.error.does_not_exist_error, flag);
        });
}